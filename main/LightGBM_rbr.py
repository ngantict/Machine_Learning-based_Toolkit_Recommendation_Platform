...
for i, row in df.iterrows():
    if row.isnull().any():
        ...
        # Get training and validation RMSE for the last boosting round
        train_rmse = mean_squared_error(y_train_split, model.predict(X_train_split), squared=False)
        val_rmse = mean_squared_error(y_val_split, model.predict(X_val_split), squared=False)
        ...
        predicted_values = model.predict(X_complete)
        ...
        mae = mean_absolute_error(y_train_split, model.predict(X_train_split))
        r2 = r2_score(y_train_split, model.predict(X_train_split))
